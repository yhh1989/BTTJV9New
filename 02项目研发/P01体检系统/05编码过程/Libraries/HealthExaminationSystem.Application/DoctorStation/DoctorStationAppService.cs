...
namespace Sw.Hospital.HealthExaminationSystem.Application.DoctorStation
{
    ...
    public class DoctorStationAppService : MyProjectAppServiceBase, IDoctorStationAppService
    {
        ...
        public List<ATjlCustomerRegDto> GetCustomerRegList(QueryClass query)
        {
            ...
            {
                queryCustomerRegisters = queryCustomerRegisters.Where(o => 
                o.CustomerBM == query.CustomerBM || o.Customer.VisitCard== query.CustomerBM);
            }
            ...
            if (!string.IsNullOrWhiteSpace(query.CustomerName))
                queryCustomerRegisters = queryCustomerRegisters.Where(o => o.Customer.Name == query.CustomerName);
            ...
            var result = queryCustomerRegisters.OrderByDescending(p=>p.LoginDate).ToList();
        ...
        public List<CustomerItemPicDto> GetCustomerItemPicDtos(QueryClass query)
        {
            var dto = _customerItemPicRepository.GetAll().AsNoTracking().Where(o => o.TjlCustomerRegID == query.CustomerRegId)
                .OrderBy(p=>p.ItemBMID).ThenBy(p=>p.pictures.CreationTime);
            ...
            foreach (var itemDepartment in Query.DepartmentBMList)
            {
                ...
                foreach (var itemGroup in Group)
                {
                    ...
                    foreach (var item in itemGroup.CustomerRegItem)
                    {
                        ...
                        var itemPiclist = _customerItemPicRepository.GetAll().Where(o => o.ItemBMID == item.Id);
                        foreach (var  itemPic in itemPiclist)
                        {
        ...
        public List<ATjlCustomerItemGroupViewDto> GettheCheckItemStatistics(StatisticalClass Query)
        {
            ...
            if (userBM != null && userBM.HospitalArea.HasValue && userBM.HospitalArea != 999)
            {
                CustomerItemGroup = CustomerItemGroup.Where(p => p.CustomerRegBM.HospitalArea == userBM.HospitalArea
                || p.CustomerRegBM.HospitalArea == null);
            }
        ...
        PatientExaminationCondition Condition)
        {
            ...
            var q = CustomerReg.Select(r => new PatientExaminationProjectStatisticsViewDto
            {
                ...
                ItemPrice = r.McusPayMoney == null ? 0 : (r.McusPayMoney.PersonalMoney + r.McusPayMoney.ClientMoney),
                 ...
                 ClientPrice= r.McusPayMoney == null ? 0 : r.McusPayMoney.ClientMoney,
                //实收金额
        ...
        public InterfaceBack ConvertInterface(TdbInterfaceDocWhere tdbInterfaceWhere)
        {
            ...
            using (CurrentUnitOfWork.DisableFilter(AbpDataFilters.MayHaveTenant))
            {
                using (CurrentUnitOfWork.DisableFilter(AbpDataFilters.MustHaveTenant))
                {
                    ...
                    //组合判断
                    if (tdbInterfaceWhere.GroupID.HasValue)
                    {
                        customerRegItemQuery =
                            customerRegItemQuery.Where(r => r.ItemGroupBMId == tdbInterfaceWhere.GroupID && r.ProcessState != (int)ProjectIState.GiveUp);
                    }
                    ...
                    foreach (var customerRegItem in customerRegItems)
                    {
                        ...
                        if (contrastItemls == null || contrastItemls.Count == 0)
                        {
                            ...
                            if (interfaceResultItem == null)
                            {
                                // 这样判断会多出一部分数据
                                // 因为本来就不是所有的项目都有对应
                                if (tdbInterfaceWhere.departmentID.HasValue && tdbInterfaceWhere.departmentID!=Guid.Empty)
                                {
                        ...
                        if (string.IsNullOrEmpty(interfaceResultItem.xmbs) && interfaceResultItem.inYQid == "TJZX")
                        {
                            ...
                            if (customerRegItem.ItemBM?.moneyType == (int)ItemType.Explain || customerRegItem.ItemBM?.moneyType == (int)ItemType.YinYang)
                            {
                                ...
                                var vitemstand = _itemStandardRepository.GetAll().Where(r => r.PositiveSate == 2 && customerRegItem.ItemId == r.ItemId
                                && r.TenantId == tandId).ToList();
                                if (vitemstand != null && vitemstand.Count>0)
                                {
                                    try
                                    {
                                        interfaceBack.StrBui.AppendLine($"项目：{customerRegItem.ItemName} 项目参考值:" + vitemstand.FirstOrDefault()?.Summ
                                       + ";项目结果:" + interfaceResultItem.invalue + "！");
                                       
                                        if (!vitemstand.Any(p =>p.Summ.ToString().Replace(" ", "").ToUpper() == interfaceResultItem.invalue.Replace(" ", "").ToUpper()))
                                        {
                                    ...
                                    {
                                        interfaceResultItem.xmckz = vitemstand.FirstOrDefault().Summ.ToString().Replace(" ", "");
                                    }
                                }
                                else
                                {
                        ...
                        if (interfaceResultItem.xmbs == "↓" || interfaceResultItem.xmbs == "L")
                        {
                            var sumsym = "偏低";
                            //重度等级
                        ...
                        else if (interfaceResultItem.xmbs == "↓↓" || interfaceResultItem.xmbs == "LL")
                        {
                            //重度等级
                            var sumsym = "极低";
                            if (decimal.TryParse(customerRegItem.ItemResultChar, out decimal rtItemResultChar))
                        ...
                        else if (interfaceResultItem.xmbs == "↑" || interfaceResultItem.xmbs == "H")
                        {
                            //重度等级
                            var sumsym = "偏高";
                            if (decimal.TryParse(customerRegItem.ItemResultChar, out decimal rtItemResultChar))
                        ...
                        else if (interfaceResultItem.xmbs == "↑↑" || interfaceResultItem.xmbs == "HH")
                        {
                            //重度等级
                            var sumsym = "极高";
                            if (decimal.TryParse(customerRegItem.ItemResultChar, out decimal rtItemResultChar))
                        ...
                        if (cricalset.Count > 0 && customerRegItem.CrisisVisitSate != 3)
                        {
                            ...
                            foreach (var cset in cricalset)
                            {
                                ...
                                {
                                    if (cset.ValueChar!=null && customerRegItem.ItemResultChar.Contains(cset.ValueChar))
                                    {
                                        if (cset.ValueNum.HasValue && cset.ValueNum>0)
                                        {
                                            #region 获取文本中的数值
                        ...
                        if (!string.IsNullOrWhiteSpace(interfaceResultItem.inPicDirs))
                        {
                           ...
                           //_commonAppService.SaveOpLog(createOpLogDto);
                            ...
                            //pdf报告转换
                            if (interfaceResultItem.inPicDirs.ToLower().Contains(".pdf"))
                            {
                                ...
                                foreach (var nimage in NitemImages)
                                {
                                    #region pdf转jpg
                                    var imagenew = nimage;
                                    if (nimage.ToLower().Contains(".pdf") && !nimage.Contains("ftp://"))
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            try
                                            {
                                                ...
                                                var iamges = PDFConvertToJPG(nimage, dateDirectory, 0);
                                ...
                                foreach (var nimage in NitemImages)
                                {
                                    ...
                                    if (nimage.Contains("ftp://"))
                                    {
                                        ...
                                        try
                                        {
                                            ...
                                            var finame = customerRegItem.ItemId  + System.DateTime.Now.ToString("yyMMddHHmmss") + nimage.Substring(len + 1, nimage.Length - len - 1);
                                            ...
                                            //_commonAppService.SaveOpLog(createOpLogDto);
                                            if (iamges.ToLower().Contains(".pdf"))
                                            {
                            ...
                            //http下载
                            #region 暂时不支持
                            if (interfaceResultItem.inPicDirs.Contains("http") && interfaceResultItem.inPicDirs.ToLower().Contains(".pdf"))
                            {
                                var NitemImages = itemImages.ToList();
                                ...
                                foreach (var nimage in NitemImages)
                                {
                                    ...
                                    if (iamges.ToLower().Contains(".dcm") || iamges.ToLower().Contains(".dic"))
                                    {
                                        var path = Path.GetDirectoryName(iamges);
                                        var filename = Path.GetFileNameWithoutExtension(iamges);
            ...
            using (CurrentUnitOfWork.DisableFilter(AbpDataFilters.MayHaveTenant))
            {
                using (CurrentUnitOfWork.DisableFilter(AbpDataFilters.MustHaveTenant))
                {
                    ...
                    foreach (var departmentItem in Create.Department)
                    {
                        ...
                        var itemInfos = itemInfos1.Where(p=>p.ItemBM!=null).Select(r => r.ItemBM).ToList();
        ...
        public OutStateDto UpdateIllState(UpCusIllStateDto updateClass)
        {
            ...
            var customerItem = _customerRegItemRepository.Get(updateClass.Id);
            if (updateClass.Symbol == "P")
            {
                customerItem.Symbol = updateClass.Symbol;
                customerItem.PositiveSate = (int)PositiveSate.Abnormal;
                if (string.IsNullOrEmpty(customerItem.ItemSum) || customerItem.ItemSum == "未见明显异常")
                {
                    customerItem.ItemSum = customerItem.ItemResultChar;
                }
                if (string.IsNullOrEmpty(customerItem.ItemDiagnosis) || customerItem.ItemDiagnosis == "未见明显异常")
                {
                    customerItem.ItemDiagnosis = customerItem.ItemResultChar;
                }
            ...
            {
                string iszcz = "";               
                var isShow = _basicDictionaryRepository.FirstOrDefault(o => o.Type == "CusSumSet" && o.Value == 4 )?.Remarks;
                if (!string.IsNullOrEmpty(isShow))
                {
                    iszcz = isShow;
                }
                ...
                if (iszcz != "N")
                {
                    if (!string.IsNullOrEmpty(customerItem.Stand))
                    {
                        xmckz = $"（正常值：{customerItem.Stand})";
                    }
                }
                if (updateClass.Symbol == "H")
                {
                    var sumsym = "偏高";
                    customerItem.ItemSum =
                                       $"{sumsym}({customerItem.ItemResultChar}{customerItem.Unit})↑{xmckz}";
                    customerItem.ItemDiagnosis = $"{customerItem.ItemName}{sumsym}({customerItem.ItemResultChar}{customerItem.Unit})↑{xmckz}";
                    customerItem.PositiveSate = (int)PositiveSate.Abnormal;
                }
                else if (updateClass.Symbol == "L")
                {
                    var sumsym = "偏低";
                    customerItem.ItemSum =
                              $"{sumsym}({customerItem.ItemResultChar}{customerItem.Unit})↓{xmckz}";
            ...
            //生成小结
            //CreateConclusionDto input = new CreateConclusionDto();
            //input.CustomerBM = updateClass.CustomerBM;
            //List<Guid> departl = new List<Guid>();
            //departl.Add(customerItem.DepartmentId);
            //input.Department = departl;
            //CreateConclusion(input);
        ...
        public OutStateDto UpdateCrisisSate(UpdateClass updateClass)
        {
            ...
            if (updateClass.CrisisSate == (int)CrisisSate.Abnormal)
            {
                ...
                customerItem.CrisiContent = updateClass.CrisiContent;
        ...
        public bool ResetCustomerChecked(QueryClass query)
        {
            ...
            if (Customer != null)
            {
                ...
                Customer.WXExportSate = 1;
                Customer.UploadState = 0;
                _customerRegRepository.Update(Customer);
                ////重置汇总 
        ...
        /// <param name="inFilePath">输入物理路径（E:\\pdf\\test.pdf）</param>
        /// <param name="pagenum">打印页数（E:\\img\\test.jpg）</param>
        public static List<string> PDFConvertToJPG(string inFilePath, string imageOutputPath, int pagenum)
        {
            // var path = Path.GetDirectoryName(inFilePath);//目录信息
            List<string> pathlist = new List<string>();
            var path = imageOutputPath;
            var filename = Path.GetFileName(inFilePath);//文件名
            using (var pdf = new PdfDocument(inFilePath))
            {
                var pagecount = pdf.Pages.Count;
                for (int p = 0;p < pagecount;p++)
                {
                    var dpiX = 150;
                    using (var img = pdf.SaveAsImage(p ,   dpiX, dpiX))
                    {
                        

                           var fileName = Path.Combine(imageOutputPath,  p.ToString()+ Path.ChangeExtension(filename , "jpg"));
        ...
        public List<CheckCusRegGroupDto> getCheckCusGroup(SearchCusGroupDto input)
        {
            ...
            if (input.StartDate.HasValue && input.EndtDate.HasValue)
            {
                ...
                {
                    que = que.Where(p=>p.CheckState!=1 && p.FirstDateTime>=input.StartDate 
