...
namespace Sw.Hospital.HealthExaminationSystem.Application.DoctorStation
{
    ...
    public class DoctorStationAppService : MyProjectAppServiceBase, IDoctorStationAppService
    {
        ...
        public List<ATjlCustomerItemGroupPrintGuidanceDto> GetATjlCustomerItemGroupPrintGuidanceDto(QueryClass Query)
        {
            ...
            return dto.MapTo<List<ATjlCustomerItemGroupPrintGuidanceDto>>();
        }
...

        {
            if (input.DepartmentBMList != null)
            {
                ...
                if (input.isJC.HasValue && input.isJC.Value == 1)
                {
                    customerRegs = _customerRegRepository.GetAll().AsNoTracking().
                      Where(o =>o.CustomerItemGroup.Any(n => n.FirstDateTime >= input.LastModificationTimeBign &&
                                        n.FirstDateTime <= input.LastModificationTimeEnd && input.DepartmentBMList.Contains(n.DepartmentId) )).ToList();
                }
                ...
                {
                    customerRegs = _customerRegRepository.GetAll().AsNoTracking().
                       Where(o => o.Customer.Name.Contains(input.Name) && o.CustomerItemGroup.Any(n => input.DepartmentBMList.Contains(n.DepartmentId))).ToList();
                }
                else 
                {
                    customerRegs = _customerRegRepository.GetAll().AsNoTracking().
                    ...
                    o.CustomerItemGroup.Any(n => input.DepartmentBMList.Contains(n.DepartmentId))).ToList();
                ...
                if (input.CheckState.HasValue)
                {
                    ...
                    customerRegs = customerRegs.Where(o => o.CustomerItemGroup.Any(p=> input.DepartmentBMList.Contains(p.DepartmentId) && 
                    p.CheckState == input.CheckState)).ToList();
                }
                if (!string.IsNullOrWhiteSpace(input.Name))
                    customerRegs = customerRegs.Where(r => r.Customer.Name.Contains(input.Name)).ToList();
        ...
        public List<CriticalCusDto> GetCriticalCusList(QueryClassTwo Query)
        {
            if (Query.DepartmentBMList != null)
            {
                ...
                var RegDto = RegItemDto.GroupBy(p=> new { p.CustomerRegBM.Id, p.CustomerRegBM.CustomerBM,
        ...
        public bool UpdateInspectionProject(List<UpdateCustomerItemGroupDto> Input)
        {
           ...
           List<string> logDept = new List<string>();
            var cusGroupIds = Input.Select(p => p.Id).Distinct().ToList();
            if (Input.Count > 0)
            {
                CustomerRegId = _customerItemGroupRepository.Get(Input.First().Id).CustomerRegBMId.Value;
            }
                //var cusreggrouplist = _customerItemGroupRepository.GetAll().Where(
            //    p=> cusGroupIds.Contains(p.Id)).ToList();

            //var cusRegItemIds = cusreggrouplist.SelectMany(p=>p.CustomerRegItem).Select(p => p.Id).Distinct().ToList();
            // var ItemIds = itemGroup.CustomerRegItem.Select(p => p.).Distinct().ToList();

            //var cusRegItems = _customerRegItemRepository.GetAll().Where(p =>
            //cusRegItemIds.Contains(p.Id)).ToList();
            //var ItemIds = cusRegItems.Select(p => p.ItemId).Distinct().ToList();
            //var Itemlist = _itemInfoRepository.GetAll().Where(p =>
            // ItemIds.Contains(p.Id)).ToList();
            ...
            foreach (var itemGroup in Input)
            {
                ...
                //var GroupDt = cusreggrouplist.FirstOrDefault(p=>p.Id== itemGroup.Id);
                var GroupDt = new TjlCustomerItemGroup();
                GroupDt.CheckState = itemGroup.CheckState; //状态
                //if (!logDept.Contains(itemGroup.))
                //{
                //    logDept.Add(itemGroup.DepartmentName);
                //}
                ...
                {
                    //GroupDt.InspectEmployeeBMId = null;
                ...
                {
                    //GroupDt.CheckEmployeeBMId = null;
                ...
                _customerItemGroupRepository.Update(itemGroup.Id, e =>
                {
                    e.CheckState = itemGroup.CheckState;
                    e.BillingEmployeeBMId = itemGroup.BillingEmployeeBMId;
                    e.InspectEmployeeBMId = itemGroup.InspectEmployeeBMId;
                    e.CheckEmployeeBMId = itemGroup.CheckEmployeeBMId;
                    e.FirstDateTime = itemGroup.FirstDateTime;
                     
                });
                //CustomerRegId = Guid.Parse(CustomerRegId);
                ...
                foreach (var item in itemGroup.CustomerRegItem)
                {
                   ...
                   // var ItemSe = cusRegItems.FirstOrDefault(p=>p.Id==item.Id);
                    var ItemSe = new TjlCustomerRegItem();
                    //ItemSe.ItemId= item.i
                   ItemSe.ItemResultChar = item.ItemResultChar; //结果
                    ...
                    _customerRegItemRepository.Update(item.Id, e =>
                    {
                        e.ItemResultChar = ItemSe.ItemResultChar;
                        e.ItemDiagnosis = ItemSe.ItemDiagnosis;
                        e.InspectEmployeeBMId = ItemSe.InspectEmployeeBMId;
                        e.CheckEmployeeBMId = ItemSe.CheckEmployeeBMId;
                        e.ProcessState = ItemSe.ProcessState;
                        e.Stand = ItemSe.Stand;
                        e.Unit = ItemSe.Unit;
                        e.StandFrom = ItemSe.StandFrom;
                    });
                    //var iteminfo = _itemInfoRepository.Get(ItemSe.ItemId);
                   // var iteminfo = Itemlist.FirstOrDefault(p=>p.Id== ItemSe.ItemId);
            ...
            //}
            createOpLogDto.LogText = "保存：科室结果" ;
                ...
                foreach (var item in itemGroup.CustomerRegItem)
                {
                    ...
                    if (cricalset.Count > 0)
                    {
                        ...
                        foreach (var cset in cricalset)
                        {
                            ...
                            {
                                if (FirstItem.ItemResultChar.Contains(cset.ValueChar))
                                {
                                    if (cset.ValueNum.HasValue)
                                    {
                                        #region 获取文本中的数值
                                        string str = cset.ValueChar;
        ...
        {
            var dto = _itemStandardRepository.GetAll();
            ...
            //{
                using (CurrentUnitOfWork.DisableFilter(AbpDataFilters.MayHaveTenant))
                {
                    ...
                    foreach (var customerRegItem in customerRegItems)
                        ...
                        var contrastItemls = _interfaceItemComparison.GetAll().Where(r => r.ItemInfoId == customerRegItem.ItemId && r.TenantId == tandId).ToList();
                        ...
                        if (interfaceResultItem == null || (string.IsNullOrWhiteSpace(interfaceResultItem.invalue) && string.IsNullOrWhiteSpace(interfaceResultItem.initemchar)))
                        {
                            ...
                            if (interfaceEmployee == null)
                            {
                                ...
                                //yhh修改没有医生创建医生
                                var userName = _userRepository.FirstOrDefault(r => r.Name == interfaceResultItem.indoctorid && r.TenantId==tandId);
                                if (userName != null)
                                {
                                ...
                                else
                                {
                                    ...
                                    var createUserFormDto = new CreateUserFormTeandIDDto
                                    {
                                        ...
                                        EmployeeNum = System.DateTime.Now.ToString("yyMMddHHmmss"),
                                        Name = interfaceResultItem.indoctorid,
                                        Password = "123qwe",
                                        HelpChar = "",
                                        Sex = 1,
                            ...
                            if (interfaceEmployeeSh == null)
                            {
                                ...
                                //yhh修改没有医生创建医生
                                var userName = _userRepository.FirstOrDefault(r => r.Name == interfaceResultItem.inSHdoctorid && r.TenantId == tandId);
                                if (userName != null)
                                {
                                ...
                                else
                                {
                                    ...
                                    var createUserFormDto = new CreateUserFormTeandIDDto
                                    {
                                        ...
                                        EmployeeNum = System.DateTime.Now.ToString("yyMMddHHmmss") + "1",
                                        Name = interfaceResultItem.inSHdoctorid,
                                        Password = "123qwe",
                                        HelpChar = "",
                                        Sex = 1,
                        ...
                        if (customerRegItem.ItemResultChar.Length > 101)
                        {
                                        ...
                                        if (nimage.Contains(".pdf") && !nimage.Contains("ftp://"))
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                try
                                                {
                                                    ...
                                                    var iamges = ConvertPDF2Image(nimage, path + "\\", filename, 1, 5, ImageFormat.Jpeg, Definition.Five);
                                                    itemImages.Remove(nimage);
                                                    itemImages.AddRange(iamges);
                                                    //createOpLogDto.LogBM = customerReg.CustomerBM;
                                                    //createOpLogDto.LogName = customerReg.Customer.Name;
                                    ...
                                    if (nimage.Contains("ftp://"))
                                    {
                                        ...
                                        //{
                                            ...
                                            try
                                            {
                                                ...
                                                var finame = nimage.Substring(len + 1, nimage.Length - len - 1);
                                                ...
                                                if (iamges.Contains(".pdf"))
                                                {
                                                    ...
                                                    var iamgest = ConvertPDF2Image(iamges, path + "\\", filename, 1, 5, ImageFormat.Jpeg, Definition.Five);
                                                    itemImages.Remove(imagenew);
                                                    itemImages.AddRange(iamgest);
                                                }
                                                //将docm图片转成jpg图片路径
                                                else if (iamges.Contains(".dcm"))
                                                {
                                                    var path = Path.GetDirectoryName(iamges);
                                                    var filename = Path.GetFileNameWithoutExtension(iamges);
                                                ...
                                                _commonAppService.SaveOpLog(createOpLogDto);
                                                DocmConvertToImagePath(docfile, fullamge, 1);
                                                    itemImages.Remove(imagenew);
        ...
        public bool CreateConclusion(CreateConclusionDto Create)
        {
            ...
            using (CurrentUnitOfWork.DisableFilter(AbpDataFilters.MayHaveTenant))
            {
                using (CurrentUnitOfWork.DisableFilter(AbpDataFilters.MustHaveTenant))
                {
                    ...
                    //图像科室（去掉）
                    // var strjcDepartment = _basicDictionaryRepository.FirstOrDefault(o => o.Type == "PresentationSet放弃" && o.Value == 70 && o.TenantId==TenantId)?.Remarks;
                    var strjcDepartment = "";
        ...
        /// <param name="Query"></param>
        /// <returns></returns>
        public bool JudgeLocking(QueryClass Query)
        {
            return _customerRegRepository.GetAll().Any(o => o.Id == Query.CustomerRegId && o.SummLocked == (int)SDState.Locking);
        }
        /// <summary>
        /// 根据条码号查询体检号
        /// </summary>
        ...
        public bool DocmConvertToImagePath(string docmpath, string strimagepath, int inum)
        {
