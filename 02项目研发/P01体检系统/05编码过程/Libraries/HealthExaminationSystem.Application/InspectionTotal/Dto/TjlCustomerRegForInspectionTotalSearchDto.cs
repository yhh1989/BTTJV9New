...
#endif
    public class TjlCustomerRegForInspectionTotalSearchDto : EntityDto<Guid>
    {
        /// <summary>
            ...
            get
            {
                if (CustomerItemGroup!=null  && CustomerItemGroup.Count>0)
                {
                    ...
                    var nocheck = CustomerItemGroup.Where(P => !P.DepartmentBM.Category.Contains("耗材")
                     && P.CheckState == (int)ProjectIState.Not && P.IsAddMinus!=3 ).Select(p => p.DepartmentName).Distinct().ToList();
