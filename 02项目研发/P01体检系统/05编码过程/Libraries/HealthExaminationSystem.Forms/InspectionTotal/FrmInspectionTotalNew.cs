    ...
    public partial class FrmInspectionTotalNew : UserBaseForm
    {
        ...
        private string ItemLoadstrFj(List<CustomerRegisterItemDto> dtoNow, bool isZYYC = false, bool isNomal = false)
        {
            ...
            for (var num = 0; num < deparsum.Count; num++)
            {
                ...
                int xh = 1;
                List<fhpd> fhlist = new List<fhpd>();
                for (int i = 0; i < dto.Count; i++)
                {

                    //复合判断只显示一次项目结果
                    if (fhlist.Any(p=>p.zuID== dto[i].ItemGroupBMId.Value 
                    && p.Text.Trim() == dto[i].ItemDiagnosis))
                    {
                        continue;
                    }
                    ...
                    {
                        fhpd fhpd = new fhpd();
                        fhpd.zuID = dto[i].ItemGroupBMId.Value;
                        fhpd.Text = dto[i].ItemDiagnosis;
                        fhlist.Add(fhpd); }
                }
                if (isDepart == true)
                {
        ...
        private void butDel_Click(object sender, EventArgs e)
        {
            ...
            {
                var currentItem = CustomerSummarizeList.FirstOrDefault(p => p.SummarizeAdviceId == departments[0]);
