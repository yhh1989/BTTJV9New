...
namespace Sw.Hospital.HealthExaminationSystem.DoctorStation
{
    public partial class frmHistory : UserBaseForm
    {
        ...
        public frmHistory(Guid _cusId, string _IDcard, Guid _departId) : this()
        {
            cusId = _cusId;
            IDcard = _IDcard;
            departId = _departId;
        ...
        private void butsearchHis_Click(object sender, EventArgs e)
        {
            ...
            {
                var itemBmlist = HisResult.Select(p => p.ItemBM).Distinct().ToList();
                string stritemBm = string.Join("','", itemBmlist);
                searchHisClassDto.ItemBM = stritemBm;
                var HisResult1 = _HistoryComparisonAppService.geHisvard(searchHisClassDto);

                foreach (var item in HisResult)
                {
                    var Itemlsit = HisResult1.FirstOrDefault(p=>p.ItemBM== item.ItemBM);
                    if (Itemlsit != null && Itemlsit.historyResultDetailDtos.Count > 0)
                    {
                        foreach (var olditem in Itemlsit.historyResultDetailDtos)
                        {
                            item.historyResultDetailDtos.Add(olditem);
                        }
                    }
