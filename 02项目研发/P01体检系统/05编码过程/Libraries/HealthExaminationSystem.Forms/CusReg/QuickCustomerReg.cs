...
{
    public partial class QuickCustomerReg : UserBaseForm
    {
        ...
        private void butAddReg_Click(object sender, EventArgs e)
        {
            ...
            //清空控件数据、清空当前登记人信息
            ClearControlData();
            DateChekDate.EditValue = DateTime.Now;//测试人员要求默认今天
            try
            {
            ...
            try
            {
                ...
                if (HISjk == "1")
                {
                    ...
                    else if (HISName.Contains("江苏鑫亿"))
                    {
                        ...
                        WindowsFormsApp1.XYNeInterface neInterface = new WindowsFormsApp1.XYNeInterface();
                        neInterface.CreateApAsync(curCustomRegInfo.Customer.ArchivesNum, data.CustomerBM, CurrentUser.EmployeeNum, CurrentUser.Name+ nm);
                    }
        ...
        private void LoadCustomerData(QueryCustomerRegDto data, Image photo = null)
        {
            ...
            if (data != null)
            {
                ...
                if (data.Id != Guid.Empty)
                {
                    ...
                    if (int.TryParse(data.Customer.CustomerTrade, out int outrTrade1))
                    {
                        txtCustomerTrade.EditValue = outrTrade1;
                    }
                    ...
                    DateChekDate.EditValue = DateTime.Now;

                if (data.PhysicalType.HasValue)
                {
                    lookUpEditClientType.EditValue = data.PhysicalType;
                }
                if (int.TryParse(data.Customer.CustomerTrade, out int outrTrade))
                {
        ...
        private void LoadControlBindData()
        {
            ...
            //客户类别
            //txtPersonnelCategory.Properties.DataSource = DefinedCacheHelper.GetBasicDictionary().Where(o => o.Type == BasicDictionaryType.CustomerType.ToString())?.ToList();
            //txtPersonnelCategory.EditValue = DefinedCacheHelper.GetBasicDictionary().Where(o => o.Type == BasicDictionaryType.CustomerType.ToString())?.ToList().FirstOrDefault().Value;
            //人员类别
        ...
        private void ClearControlData()
        {
            ...
            lookUpEditClientType.EditValue = DefinedCacheHelper.GetBasicDictionary().Where(o => o.Type == BasicDictionaryType.ExaminationType.ToString())?.ToList().FirstOrDefault().Value;
            txtCustomerTrade.EditValue = null;
            //txtPersonnelCategory.EditValue = DefinedCacheHelper.GetBasicDictionary().Where(o => o.Type == BasicDictionaryType.ExaminationType.ToString())?.ToList().FirstOrDefault().Value;
            //加载默认查询条件
            string fp = System.Windows.Forms.Application.StartupPath + "\\CustomerReg.json";
            if (File.Exists(fp))  // 判断是否已有相同文件 
            {
            ...
            var Clientlndutry = DefinedCacheHelper.GetBasicDictionary().Where(o =>
            o.Type == clientlndutry).ToList();
            var Clientlndutrytjlb = Clientlndutry.Where(o => o.Remarks.Contains(lookUpEditClientType.Text)).ToList();
            if (Clientlndutrytjlb.Count > 0)
            { txtCustomerTrade.Properties.DataSource = Clientlndutrytjlb; }
            else
            {
                txtCustomerTrade.Properties.DataSource = Clientlndutry;
            }
        ...
        private QueryCustomerRegDto GetControlSaveData(bool isSave = false)
        {
            ...
            input.MarriageStatus = (int)MarrySate.Unmarried;
            ...
            if (txtCustomerTrade.EditValue !=null)
            {
                input.Customer.CustomerTrade = txtCustomerTrade.EditValue.ToString();
            }
            else
            {
                input.Customer.CustomerTrade = null;
            }
        ...
        private void QueryRegNumbers()
        {
            try
            {
                ...
                var str = "今日数据：总人数：" + numbers.SumReg + "<br>";
                foreach (var data in numbers.datas)
                {
                    str += data.ClientName + ":总数：" + data.Sum
                        + "(男:" + data.Male
                        + "，女：" + data.Famale + ")<br>";
                    //+ "，未说明的性别：" +data.NoSex;
                }
        ...
        private void txtAge_Leave(object sender, EventArgs e)
        {
            ...
            var year = DateTime.Now.Year - Convert.ToInt32(txtAge.Text);
        ...
        private string GetBTPD(string tjcode)
        {
            ...
            if (strPD == "Y")
            {
                ...
                else
                {
                    ...
                    //    "' and time<'"+DateTime.Now.ToString("yyyy-MM-dd")+" 00:00:02");
        ...
        private void lookUpEditClientType_EditValueChanged_1(object sender, EventArgs e)
        {
