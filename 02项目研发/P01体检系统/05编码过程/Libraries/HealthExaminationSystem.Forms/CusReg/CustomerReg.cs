                ...
                if (dr == DialogResult.OK)
                {
                    ...
                    {//个人登记添加套餐的情况
                        foreach (var item in suitInfo.ItemSuitItemGroups)
                        {
                            ...
                            if (curCustomRegInfo.RegisterState == (int)RegisterState.No)
                            {
                                var first = curCustomRegInfo.CustomerItemGroup.FirstOrDefault(o => o.ItemGroupBM_Id == group.ItemGroupBM_Id);


                                first.ItemPrice = group.ItemPrice;
                                first.PriceAfterDis = group.PriceAfterDis;
                                first.DiscountRate = group.DiscountRate;
                                first.GRmoney = group.GRmoney;
                                first.IsAddMinus = group.IsAddMinus;
                                first.PayerCat = group.PayerCat;
                                first.TTmoney = group.TTmoney;
                                first.ItemSuitId = group.ItemSuitId;
                                first.ItemSuitName = group.ItemSuitName;
                                first.SFType = group.SFType;
                                first.DepartmentId = group.DepartmentId;
                                first.DepartmentName = group.DepartmentName;
                                first.DepartmentOrder = group.DepartmentOrder;                                
...
" +  data.ClientName + ":总数：" + data.Sum
        ...
        private void txtAge_Leave(object sender, EventArgs e)
        {
            ...
            var nowtime = _commonAppService.GetDateTimeNow().Now;
            var year = nowtime.Year - Convert.ToInt32(txtAge.Text);
            //还没到月份
            if (DateBirthday.DateTime.Month > nowtime.Month)
            {
                year = year - 1;
            }
            else if (DateBirthday.DateTime.Month == nowtime.Month &&
                DateBirthday.DateTime.Day > nowtime.Day)
            { year = year - 1; }
        ...
        private void simpleButton6_Click(object sender, EventArgs e)
        {
            ...
            else
            {
               ...
               var    picnow = pic.FirstOrDefault(p => p.Contains("jpg"));
                pictureCusReg.LoadAsync(picnow);
            }
        ...
        {
            jsface = true;
            newTime = _commonAppService.GetDateTimeNow().Now;
            getfaceInfo();
            timer1.Enabled = true;
            timer1.Start();
        }
        ...
        {
            // 创建网络连接对象
            NetworkInterface networkInterface = NetworkInterface.GetAllNetworkInterfaces().FirstOrDefault(n => n.OperationalStatus == OperationalStatus.Up && !n.Description.Contains("Loopback"));

            if (networkInterface != null)
            {
                // 遍历所有网络接口上的 IPv4 配置信息
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            if (newTime.HasValue)
            {
                var starTime = _commonAppService.GetDateTimeNow().Now;
                TimeSpan sjc = starTime - newTime.Value;
                if (sjc.TotalMinutes > 10)
                {
                    timer1.Stop();
                    timer1.Enabled = false;
                    labelClientInfo.Text = "获取人脸设备信息超时";
                }
            }
            if (jsface == true)
            {
                labelClientInfo.Text = "正在检索人脸设备信息";
                getfaceInfo();
            }
        }
        private void getfaceInfo()
        {
            var RLJK = DefinedCacheHelper.GetBasicDictionaryByValue(BasicDictionaryType.FaceSet, 1)?.Remarks;
            if (string.IsNullOrEmpty(RLJK))
            {
                MessageBox.Show("请先在字典，人脸设备中开启人脸设备接口，配置：(值：1，文本：开启人脸设备，备注：Y)");
                jsface = false;
                timer1.Stop();
                timer1.Enabled = false;
                labelClientInfo.Text = "";
            ...
            var url = DefinedCacheHelper.GetBasicDictionaryByValue(BasicDictionaryType.FaceSet, 2)?.Remarks;
            if (string.IsNullOrEmpty(url))
            {
                MessageBox.Show("请先在字典，人脸设备中设置人脸设备URL，配置：(值：2，文本：人脸设备URL，备注：人脸设备中配置的接口地址)");
                timer1.Stop();
                timer1.Enabled = false;
                jsface = false;
                labelClientInfo.Text = "";
            ...
            var IPSHB = DefinedCacheHelper.GetBasicDictionary().Where(p => p.Type == BasicDictionaryType.FaceSetIP.ToString()).ToList();
            ...
            var bjsbh = IPSHB.FirstOrDefault(p => p.Text == ip)?.Remarks;
            if (!string.IsNullOrEmpty(bjsbh))
            {
                labelClientInfo.Text = "开始检索人脸设备：" + bjsbh+"数据";
                dynamic input = new { SBH = bjsbh };
                var cusInfolst = customerSvr.getFace(input);
                string ss = Convert.ToString(cusInfolst);
                if (cusInfolst != null)
                {
                    var cusInfo = cusInfolst;
                    if (cusInfo != null)
                    {
                        labelClientInfo.Text = "人脸设备数据检索成功，正在加载，请稍等";
                        if (cusInfo.cDsex != null)
                        {
                            int sexbm = 1;
                            if (cusInfo.cDsex == "女")
                            { sexbm = 2; }
                        ...
                        if (cusInfo.cDfaceImg != null)
                        {
                            string ssp = Convert.ToString(cusInfo.cDfaceImg);
                            byte[] bytes = Convert.FromBase64String(ssp);
                            MemoryStream ms = new MemoryStream(bytes);
                            Bitmap bitmapImage1 = new Bitmap(ms);
                            pictureCus.Image = bitmapImage1;
                        }
                        if (cusInfo.cDidentityId != null)
                        {
                            txtIDCardNo.EditValue = cusInfo.cDidentityId;
                        }
                        if (cusInfo.cDbirthday != null)
                        {
                            DateBirthday.EditValue = cusInfo.cDbirthday;
                        }
                        //民族
                        if (cusInfo.cDnation != null)
                        {
                            txtNation.EditValue = cusInfo.cDnation;
                        }
                        //地址
                        if (cusInfo.cDaddress != null)
                        {
                            if (curCustomRegInfo != null && curCustomRegInfo.Customer != null)
                            {
                                curCustomRegInfo.Customer.Address = cusInfo.cDaddress;
                            }
                        }
                        //姓名
                        if (cusInfo.person_name != null)
                        {
                            txtName.EditValue = cusInfo.person_name;
                        }
                        if (cusInfo.capture_img != null)
                        {
                            string ssp = Convert.ToString(cusInfo.capture_img);
                            byte[] bytes = Convert.FromBase64String(ssp);
                            MemoryStream ms = new MemoryStream(bytes);
                            Bitmap bitmapImage1 = new Bitmap(ms);
                            pictureCusReg.Image = bitmapImage1;
                        }
                        labelClientInfo.Text = "人脸数据加载成功";
                        timer1.Stop();
                        timer1.Enabled = false;
                        jsface = false;

                        dynamic inputcs = new { SBH = bjsbh };
                        var cuslis = customerSvr.UpFace(inputcs);
            ...
            {
                MessageBox.Show("为找到匹配本机IP的人脸设备，请先在字典，人脸设备对应中设置，配置：(文本：本机IP，备注：人脸设备号)");
                timer1.Stop();
                timer1.Enabled = false;
                jsface = false;
            }
        ...
        private void simpleButton12_Click(object sender, EventArgs e)
        {
            ...
            frmSearchClient frmSearch = new frmSearchClient();
            if (frmSearch.ShowDialog() == DialogResult.OK && frmSearch.clientInfoesListInput !=null 
                && !string.IsNullOrEmpty(frmSearch.clientInfoesListInput.ClientName))
            {
                textEditClientname.Text = frmSearch.clientInfoesListInput.ClientName;
                textEditClientCode.Text = frmSearch.clientInfoesListInput.SocialCredit;
            }
        }

        private void textEditClientname_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter && !string.IsNullOrWhiteSpace(textEditClientname.Text))
            {
                frmSearchClient frmSearch = new frmSearchClient();
                frmSearch.clientName = textEditClientname.Text;
                if (frmSearch.ShowDialog() == DialogResult.OK && frmSearch.clientInfoesListInput != null
                    && !string.IsNullOrEmpty(frmSearch.clientInfoesListInput.ClientName))
                {
                    textEditClientname.Text = frmSearch.clientInfoesListInput.ClientName;
        ...
        //                    MemoryStream ms = new MemoryStream(bytes);
        //                    Bitmap bitmapImage1 = new Bitmap(ms);
        //                    faceImg = bitmapImage1;
        //                    //pictureCus.Image = bitmapImage1;
        //                }
        //                string carID = "";
        //                if (cusInfo.cDidentityId != null)
        //                {
        //                    carID = (string)cusInfo.cDidentityId;
        //                   // txtIDCardNo.EditValue = cusInfo.cDidentityId;
        //                }
        //                //DateTime brth = null;
        //                if (cusInfo.cDbirthday != null)
        ...
        //                if (cusInfo.cDaddress != null)
        //                {
        //                    //DateBirthday.EditValue = cusInfo.card_data.address;
        //                }
        //                //姓名
        //                if (cusInfo.person_name != null)
        //                {
