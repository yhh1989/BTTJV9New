...
{
    public partial class FrmInspectionTotalNew : UserBaseForm
    {
        ...
        private void InitForm()
        {
            ...
            #endregion
            // 参数控制显示上传列
            var isshow = DefinedCacheHelper.GetBasicDictionaryByValue(BasicDictionaryType.CusSumSet, 15)?.Remarks;
        ...
        private void gridView1_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            ...
            gridView3.ActiveFilter.Remove(gridColumn34);
            //treeListZhenDuan.DataSource = null;
            if (gridViewCusReg.IsDataRow(e.FocusedRowHandle))
            {
                if (gridViewCusReg.GetFocusedRowCellValue(conId) is Guid id)
                {
                    TS = "";
                 ...
                 new ColumnFilterInfo("[MinAge] <=" + _tjlCustomerRegDto.Age + ""));
                        ...
                        #endregion
                        gridControl2.DataSource = customerRegisterDepartmentSummaryTask.Result;
        ...
        private void makeSum(int isOK = 1)
        {
            ...
            if (gridControl2.DataSource is List<CustomerRegisterDepartmentSummaryDto> field科室小结汇总)
            {
                ...
                else
                {
                    ...
                    {
                        foreach (var customerRegisterDepartmentSummaryDto in field科室小结汇总)
                        {
                    ...
                    else
                    {
                        ...
                        if (IsYC != null && IsYC.Remarks == "0")
                        {
                            ...
                            if (IsYCgjc != null && IsYCgjc.Remarks != "")
                            {
                                ...
                                field科室小结汇总 = field科室小结汇总.Where(p => !gjcls.Contains(p.DagnosisSummary.Replace(" ", ""))).ToList();
        ...
        private string ItemLoadstrFj(List<CustomerRegisterItemDto> dtoNow, bool isZYYC = false,bool isNomal = false)
        {
           ...
           // bool isNomal = false;
            //if (dtoNow.Any(p => p.CrisisSate ==null || p.CrisisSate == (int)CrisisSate.Normal ))
            //{
            //    isNomal = true;
            //}
            int ZXH = 1;
                ...
                for (int i = 0; i < dto.Count; i++)
                {
                   ...
                   //ZjFormat = ZjFormat.Replace("【总序号】", (ZXH).ToString());
                    ...
                    if (ZjFormat != "" && !string.IsNullOrWhiteSpace(itemSum))
                    {
                        //if (isZYYC == true)
                        //{
                        //    ZjFormat = ZjFormat.Replace("【换行】", "").Replace("【空格】", "");
                        //}
                        //else
                        //{
                            ZjFormat = ZjFormat.Replace("【换行】", Environment.NewLine).Replace("【空格】", " ");
        ...
        private void simpleButtonXinZeng2_Click(object sender, EventArgs e)
        {
            ...
            treeListZhenDuan.FocusedColumn = treeListColumn5;
        ...
        {
            //上
            var list = treeListZhenDuan.DataSource as List<CustomerRegisterSummarizeSuggestDto>;
            if (list == null)
                return;
            var currentItem = treeListZhenDuan.GetFocusedRow() as CustomerRegisterSummarizeSuggestDto;
            if (currentItem == null)
                return;
        ...
        private void butDel_Click(object sender, EventArgs e)
        {
            ...
            var deparNames = new List<string>();

            foreach (TreeListNode node in treeListZhenDuan.Nodes)
            {
                if (node.Checked)
                {
                    var NODEid = node.GetValue(conSummarizeAdviceId);
                    if (NODEid != null)
                    {
                        var id = (Guid)node.GetValue(conSummarizeAdviceId);
                        departments.Add(id);
                    }
                    else
                    {
                        var departName = node.GetValue(conSummarizeName).ToString();
                        deparNames.Add(departName);
            ...
            if (departments.Count == 0 && deparNames.Count == 0)
            {
                XtraMessageBox.Show("请选择删除行！");
                return;
            }
            var CustomerSummarizeList = list;// treeListZhenDuan.DataSource as List<CustomerRegisterSummarizeSuggestDto>;
            ...
            {
                var currentItem = CustomerSummarizeList.FirstOrDefault(p => p.Id == departments[0]);
                currentIndex = list.IndexOf(currentItem);
            }
            else
            {
                var currentItem = CustomerSummarizeList.FirstOrDefault(p => p.SummarizeName == deparNames[0]);
                currentIndex = list.IndexOf(currentItem);
            }
            //将删除行的所有子级行变为普通行
            //for (int i = 0; i < list.Count; i++)
            //{
            //    if (list[i].ParentAdviceId.HasValue && departments.Contains(list[i].ParentAdviceId.Value))
            //    {
            //        list[i].ParentAdviceId = null;
            //        list[i].SummarizeName = list[i].SummarizeName;
            //    }
            //    if (departments.Contains(list[i].SummarizeAdviceId.Value
            //        ))
            //    {
            //        CustomerSummarizeList.Remove(list[i]);
            //    }
            //}
            if (departments.Count > 0)
            {
                CustomerSummarizeList = CustomerSummarizeList.Where(p => p.SummarizeAdviceId == null || (p.SummarizeAdviceId != null && !departments.Contains(p.SummarizeAdviceId.Value))).ToList();

            }
            if (deparNames.Count > 0)
            {
                CustomerSummarizeList = CustomerSummarizeList.Where(p => !deparNames.Contains(p.SummarizeName)).ToList();

            }

            treeListZhenDuan.DataSource = OrderByList(CustomerSummarizeList);
            // treeListZhenDuan.BestFitColumns();
            treeListZhenDuan.RefreshDataSource();
            if (currentIndex > 1)
            {
        ...
        private void simpleButtonShenHe_Click(object sender, EventArgs e)
        {
            ...
            {
                //更新建议汇总
                customerRegisterSummarizeDto.ShEmployeeBMId = (long)customyiSheng.EditValue;
                if (!customerRegisterSummarizeDto.EmployeeBMId.HasValue)
                {
                    customerRegisterSummarizeDto.EmployeeBMId = (long)customyiSheng.EditValue;
                }
        ...
        private void LoadDataState()
        {
            ...
            if (_tjlCustomerRegDto.SummSate == (int)SummSate.HasInitialReview || _tjlCustomerRegDto.SummSate == (int)SummSate.Audited)
            {
                ...
                if (CurrentUser.Id == _tjlCustomerRegDto.CSEmployeeId || _tjlCustomerRegDto.FSEmployeeId == CurrentUser.Id || IsTrue)
                {
                    if (_tjlCustomerRegDto.SummSate == (int)SummSate.Audited)
                    {
                        ...
                        //已审核但是不是审核医生无权清除
                        if (!IsTrue && _tjlCustomerRegDto.FSEmployeeId.HasValue && _tjlCustomerRegDto.FSEmployeeId != CurrentUser.Id)
                        {
            ...
            else if (NowCusRegInf.SummLocked == (int)SummLockedState.Alr && NowCusRegInf.SummLockEmployeeBMId != CurrentUser.Id)
            {
                //已锁定
                //BtnSave.Enabled = false;
                //simpleButtonShengCheng.Enabled = false;
        ...
        private void gridView2_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            ...
            {
                if (gridView2.GetFocusedRowCellValue(gridColumn35) is Guid id)
                {
                    string depsear = "[DepartmentId] ==" + id + "";
        ...
        private void butUpdate_Click(object sender, EventArgs e)
        {
            ...
            {
                //获取原有建议
                var OldSum = treeListZhenDuan.DataSource as List<CustomerRegisterSummarizeSuggestDto>;
                if (OldSum == null)
                {
        ...
        {
            GridHitInfo info = gridView2.CalcHitInfo(e.X, e.Y);

            if (info.RowHandle < 0)

            {
                gridView3.ActiveFilter.Remove(gridColumn34);  //清空选择
        ...
        private void gridView9_RowClick(object sender, DevExpress.XtraGrid.Views.Grid.RowClickEventArgs e)
        {
            if (e.Clicks == 2)
            {
                var RowDataList = gridSelectAdvice.GetSelectedRowDtos<SummarizeAdviceDto>();
                if (RowDataList == null)
                {
                    return;
                }
        ...
        private void richTextBoxAdvice_TextChanged(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(richTextBoxAdvice.Text))
            {
                var advicelist = DefinedCacheHelper.GetSummarizeAdvices().Where(
                    p => p.AdviceName.Contains(richTextBoxAdvice.Text) || p.Advicevalue.Contains(richTextBoxAdvice.Text)).
                    OrderBy(p => p.OrderNum).ToList();
                gridSelectAdvice.DataSource = advicelist;
        ...
        private void allCheck_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                treeListZhenDuan.CheckAll();
               // SelectTreeListAll(treeListZhenDuan);
                allCheck.Text = "全不选";
            }
            else
            {
                treeListZhenDuan.UncheckAll();
                //NOSelectTreeListAll(treeListZhenDuan);
                allCheck.Text = "全选";
            }
        }
