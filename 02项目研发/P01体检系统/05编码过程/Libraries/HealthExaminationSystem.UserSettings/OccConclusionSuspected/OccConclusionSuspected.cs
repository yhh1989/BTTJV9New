...
using Sw.Hospital.HealthExaminationSystem.ApiProxy.OccConclusionSuspected;
...
using static Sw.Hospital.HealthExaminationSystem.UserSettings.OccDisRequisition.OccDisRequisitionList;

namespace Sw.Hospital.HealthExaminationSystem.UserSettings.OccConclusionSuspected
{
    public partial class OccConclusionSuspected : UserBaseForm
    {
        private readonly IOccConclusionSuspectedAppService _OccConclusionSuspectedAppService;
        private readonly IClientInfoesAppService _IClientInfoesAppService;

        public OccConclusionSuspected()
        {
            InitializeComponent();
            _OccConclusionSuspectedAppService = new OccConclusionSuspectedAppService();
            _IClientInfoesAppService = new ClientInfoesAppService();
        }

        private void OccConclusionSuspected_Load(object sender, EventArgs e)
        {
            var occ = new OccSuspectedGet();
            ...
            var results = _OccConclusionSuspectedAppService.GetOccConclusionSuspected(occ);
        ...
        void LoadData()
        {
            try
            {
                ...
                var data = _OccConclusionSuspectedAppService.GetOccConclusionSuspected(occ);
        ...
        {
            var result = gridControl1.DataSource as List<OccConclusionSuspectedShow>;
            if (result != null && result.Count > 0)
            {
                var frm = new SuspectedChart(result);
                frm.ShowDialog();
            }
            else
            {
